...
//    final baseUrl = "https://localhost:44321/";
    ...
    login(String email, String password) {
        return post('Identity/login', jsonEncode(<String, String>{
            'Email': email,
            'Password': password,
        }));
    }
    ...
    get(String uri) async {
        ...
        if (response.statusCode == 401) {
            ...
            if (isTokenRefreshed) {
                response = await http.get(baseUrl + uri,
                    headers: {HttpHeaders.authorizationHeader: "Bearer ${state.token}"});
            }
        }
    ...
    post(String uri, postData) {
        return http.post(baseUrl + uri, body: postData, headers: <String, String>{
    ...
    Future<bool> refreshAccessToken() async {
        var response = await http.get(baseUrl + 'Identity/RefreshTokenMobile?token=${state.refreshToken}');

        if (response.statusCode == 200) {
            final responseData = json.decode(response.body);
            state.setTokens(responseData['access_token'], responseData['refresh_token']);
